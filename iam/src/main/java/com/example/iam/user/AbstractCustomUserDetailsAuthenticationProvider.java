...
public abstract class AbstractCustomUserDetailsAuthenticationProvider
  ...
  protected abstract void additionalAuthenticationChecks(
      CustomUserDetails userDetails, EmailPasswordAuthenticationToken authentication);
  ...
  public Authentication authenticate(Authentication authentication) throws AuthenticationException {
    Assert.isInstanceOf(
        EmailPasswordAuthenticationToken.class,
        authentication,
        () ->
            messages.getMessage(
    ...
    if (user == null) {
      cacheWasUsed = false;
      try {
        user = retrieveUser(email, (EmailPasswordAuthenticationToken) authentication);
  ...
  protected Authentication createSuccessAuthentication(
      Object principal, Authentication authentication, CustomUserDetails user) {
    // Ensure we return the original credentials the user supplied,
    // so subsequent attempts are successful even with encoded passwords.
    // Also ensure we return the original getDetails(), so that future
    // authentication events after cache expiry contain the details
    EmailPasswordAuthenticationToken result =
   ...
   * implementation-specific location, with the option of throwing an <code>AuthenticationException
   * </code> immediately if the presented credentials are incorrect (this is especially useful if it
   * is necessary to bind to a resource as the user in order to obtain or generate a <code>
   * UserDetails</code>).
   *
   * <p>Subclasses are not required to perform any caching, as the <code>
   * AbstractCustomUserDetailsAuthenticationProvider</code> will by default cache the <code>
   * UserDetails</code>. The caching of <code>UserDetails</code> does present additional complexity
   * as this means subsequent requests that rely on the cache will need to still have their
   * credentials validated, even if the correctness of credentials was assured by subclasses
   ...
   * ensure subclasses implement {@link #additionalAuthenticationChecks(UserDetails,
   * EmailPasswordAuthenticationToken)} to compare the credentials of a cached <code>UserDetails
   * </code> with subsequent authentication requests.
   *
   ...
   * EmailPasswordAuthenticationToken)} so that code related to credentials validation need not be
   ...
   */
  protected abstract CustomUserDetails retrieveUser(
      String email, EmailPasswordAuthenticationToken authentication);
   ...
   * BadCredentialsException</code> if a username is not found or the password is incorrect. Setting
   * this property to <code>false</code> will cause <code>UsernameNotFoundException</code>s to be
   * thrown instead for the former. Note this is considered less secure than throwing <code>
   * BadCredentialsException</code> for both exceptions.
   *
    ...
    public void check(UserDetails user) {
      ...
      if (!user.isEnabled()) {
        ...
        throw new DisabledException(
            messages.getMessage(
                "AbstractCustomUserDetailsAuthenticationProvider.disabled", "User is disabled"));
      }
